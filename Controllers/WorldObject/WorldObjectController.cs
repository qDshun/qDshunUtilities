...
namespace qDshunUtilities.Controllers.WorldObject;
...
{
    [HttpGet]
    public async Task<ActionResult<GetWorldObjectsResponse>> GetWorldObjects([FromRoute] Guid worldId)
    {
        return await worldObjectService.GetWorldObjectsAsync(worldId, AuthenticatedUser);
    }

    [HttpGet("{worldObjectId}")]
    public async Task<ActionResult<GetSingleWorldObjectResponse>> GetWorldObject([FromRoute] Guid worldId, [FromRoute] Guid worldObjectId)
    {
        return new GetSingleWorldObjectResponse(await worldObjectService.GetWorldObjectAsync(worldId, worldObjectId, AuthenticatedUser));
    }
