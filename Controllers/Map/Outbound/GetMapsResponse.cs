...
using static qDshunUtilities.Controllers.ObjectField.Outbound.GetObjectFieldsResponse;

namespace qDshunUtilities.Controllers.Map.Outbound
{
    public class GetMapsResponse(IEnumerable<MapModel> models)
    {
        public IEnumerable<MapDto> Maps { get; set; } = models.Select(model => new MapDto(model));
        public class MapDto(MapModel model)
        {
